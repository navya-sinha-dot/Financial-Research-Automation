"""Tests for the ingestion layer: scraping, HTML persistence, parsing, and failure isolation."""
import os
import pytest
from pathlib import Path
from unittest.mock import patch

from src.ingestion.scraper import save_raw_html, generate_fallback_financial_html
...
def test_save_raw_html_creates_file(tmp_path):
    with patch("src.ingestion.scraper.settings.RAW_HTML_DIR", tmp_path):
        sample_html = "<html><body><h1>Sample</h1></body></html>"
        saved = save_raw_html("TEST", sample_html)
        assert saved.exists()
        assert "TEST" in saved.name
...
def test_batch_ingestion_failure_isolation(db_session):
    ...
    with patch("src.ingestion.tasks.fetch_company_financials_html") as mock_fetch:
        def side_effect(ticker):
            ...
            return generate_fallback_financial_html(ticker), Path(f"data/raw_html/{ticker}.html")
